...
import ar.com.wecoode.jitws.service.SucursalService;
...
public class SucursalController {
    ...
    //Define la url
    private final String URL = RutaConstant.URL_BASE + "/sucursal";
    //Define la url de subcripciones a sockets
    private final String TOPIC = RutaConstant.URL_TOPIC + "/sucursal";
    ...
    @ResponseBody
    public List<Sucursal> listarPorNombre(@PathVariable String nombre) {
        return elementoService.listarPorNombre(nombre);
    }
    
    //Agrega un registro
    @PostMapping(value = URL)
    public ResponseEntity<?> agregar(@RequestBody Sucursal elemento) {
        try {
            Sucursal e = elementoService.agregar(elemento);
            ...
            if(partes[3].equals(DuplicidadError.NOMBRE_UNICO)) {
                ...
                return new ResponseEntity(new EstadoRespuesta(CodigoRespuesta.DATO_DUPLICADO_NOMBRE, 
                    MensajeRespuesta.DATO_DUPLICADO + " '" + elemento.getNombre() + " - " 
                            + elemento.getLocalidad().getNombre() + "'"), 
    ...
    public ResponseEntity<?> actualizar(@RequestBody Sucursal elemento) {
        try {
    ...
    public ResponseEntity<?> eliminar(@RequestBody Sucursal elemento) {
        try {
