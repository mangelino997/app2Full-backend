...
//    }
    //Obtiene el precio del flete
    public BigDecimal obtenerPrecioFlete(int idOrdenVenta, int idTipoTarifa, String valor) {
        BigDecimal v = new BigDecimal(valor);
...
//        BigDecimal valorHasta = escalaTarifas.get(0).getValor().subtract(escalaTarifas.get(1).getValor()).setScale(2, RoundingMode.UNNECESSARY);
        ...
        //Si importeFijo es null, devuelve el importe unitario por el valor o el importe minimo - devuelve el mayor
        return ove.getImporteFijo() != null ? ove.getImporteFijo()
                : (v.multiply(ove.getPrecioUnitario()).compareTo(ove.getMinimo()) > 0
                ? v.multiply(ove.getPrecioUnitario()) : ove.getMinimo());
    }
    //Agrega un registro
