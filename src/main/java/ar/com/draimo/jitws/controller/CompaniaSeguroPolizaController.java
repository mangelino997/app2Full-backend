...
//            template.convertAndSend(TOPIC + "/lista", elementoService.listar());
    ...
    public ResponseEntity<?> actualizarPDF(@RequestParam("idCompaniaSeguroPoliza") int idCompaniaSeguroPoliza, 
            @RequestParam("idPdf") int idPdf, @RequestParam("archivo") MultipartFile archivo) {
        try {
            //Actualiza el registro
            Object companiaSeguroPoliza = elementoService.actualizarPDF(idCompaniaSeguroPoliza, idPdf, archivo);
            //Envia la nueva lista a los usuarios subscripto
            ////template.convertAndSend(TOPIC + "/lista", elementoService.listar());
            //Retorna mensaje de actualizado con exito
            return new ResponseEntity(companiaSeguroPoliza, HttpStatus.OK);
        } catch (Exception e) {
            //Retorna mensaje de error interno en el servidor
            return MensajeRespuesta.error();
        }
    ...
    //Elimina un registro
    @DeleteMapping(value = URL + "/eliminarPdf/{idCompaniaSeguroPoliza}/{idPdf}")
    public ResponseEntity<?> eliminarPDF(@PathVariable int idCompaniaSeguroPoliza, @PathVariable int idPdf) {
        try {
            CompaniaSeguroPoliza companiaSeguroPoliza = elementoService.eliminarPDF(idCompaniaSeguroPoliza, idPdf);
            //Retorna mensaje de eliminado con exito
            return new ResponseEntity(companiaSeguroPoliza.getVersion(), HttpStatus.OK);
        } catch (DataIntegrityViolationException dive) {
            //Retorna mensaje de dato duplicado
            return MensajeRespuesta.datoDuplicado(dive);
        } catch (Exception e) {
